...
//  Snacktacular
//
//  Created by Chloe Chen on 11/1/21.
//
    ...
    func updateAverageRating(completed: @escaping() -> ()) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error ) in
            ...
            spotRef.setData(dataToSave) { (error) in
                if let error = error {
                    print("ERROR: updating document \(self.documentID) in spot after changing averageReview & numberOfRevews info \(error.localizedDescription)")
                    completed()
                } else {
